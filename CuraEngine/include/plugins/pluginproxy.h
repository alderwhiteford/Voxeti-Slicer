        ...
        , broadcast_stub_{ channel }
    {
        ...
        boost::asio::co_spawn(
            ...
            [this, &grpc_context, &status, &plugin_info, &handshake_stub, &name, &version]() -> boost::asio::awaitable<void>
            {
                ...
                if (valid_)
                {
                    ("Using plugin: '{}-{}' running at [{}] for slot {}", plugin_info.plugin_name, plugin_info.plugin_version, plugin_info.peer, slot_info_.slot_id);
                    if (! plugin_info.broadcast_subscriptions.empty())
                    {
                        ("Subscribing plugin '{}' to the following broadcasts {}", plugin_info.plugin_name, plugin_info.broadcast_subscriptions);
                    }
                }
            },
            boost::asio::detached);
