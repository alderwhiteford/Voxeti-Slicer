...
namespace cura
{
    float layerHeightOverride = 0.2;
    void setLayerHeight(const float height)
    {
        layerHeightOverride = height;
    }
    ...
    coord_t Settings::get<coord_t>(const std::string &key) const
    {
        if (key == "layer_height")
        {
            return MM2INT(layerHeightOverride);
        }
        else
        {
    ...
    {
        auto keys = getKeys();
        return keys | ranges::views::transform([&](const auto &key)
